...
public class PuzzleEntryButton : MonoBehaviour
{
    ...
    void CheckClick()
    {
        if (GameInput.Instance == null) { Debug.Log("GameInput null"); return; }
        if (puzzleCompleted) { Debug.Log("puzzleCompleted = true, клик игнорируется"); return; }
        if (isLoading) { Debug.Log("isLoading = true"); return; }

        if (GameInput.Instance.PeekClickPosition(out Vector2 clickPos))
        {
            Debug.Log($"Peek сработал, clickPos={clickPos}, isHovered={isHovered}");
            Debug.Log($"col.OverlapPoint={col.OverlapPoint(clickPos)}");

            if (col.OverlapPoint(clickPos))
            {
                GameInput.Instance.ConsumeClick();
                StartCoroutine(LoadPuzzleAdditive());
            }
        }
        else
        {
            // Убери эту строку после диагностики — она будет спамить
            // Debug.Log("Peek — нет клика");
        }
    }
